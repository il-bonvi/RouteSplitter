...
**Fase attuale:** vento a zone completo end-to-end: bussola sulla mappa, animazione vento stile Windfinder sulla planimetria (un solo streak sottile e radi per punto, oscillazione locale breve senza linea guida di sfondo — niente più sovrapposizioni sui tornanti, colore per direzione, velocità per intensità, trasparenza costante), marker numerati delle sezioni ridotti (12px), convertitore km/h↔kts nel pannello vento, fascia testa/coda in altimetria allineata al grafico (zoom incluso), ottimizzatore di pacing wind-aware. 107 test verdi, typecheck/build puliti.
...
- **2026-08-19** — Nuova funzionalità (primo passo verso una vera visualizzazione vettoriale del vento, ispirata a Best Bike Split / Windfinder / earth.nullschool): **frecce vento statiche sulla planimetria**. Prima l'unico riferimento grafico del vento sul percorso era la `WindRibbon` (fascia 1D sopra il grafico altimetria) — non si vedeva a colpo d'occhio *dove* il vento cambia effetto lungo la strada reale, cosa che rendeva difficile decidere dove spezzare le zone vento. Discusse con l'utente 2 scelte di design prima di implementare (colore e densità) via domande a scelta rapida. **Nuovo componente** `WindArrowsLayer.tsx` (dentro `RouteMap.tsx`, montato solo se `windZones.length >= 2`): per ogni punto campionato lungo il percorso calcola il vento locale (`windAtDistKm`) e lo rende come freccia SVG (`L.divIcon`, non interattiva) ruotata sulla **direzione vera verso cui soffia il vento** (`directionDeg + 180`, stessa convenzione "blowing to" già usata da `WindCompass`), non relativa alla strada — il confronto con l'orientamento del tracciato sotto si legge già a occhio. **Colore**: rosso (testa) / verde (coda) — stessa scala della ribbon, ora estratta in `headwindColor()` condivisa in `lib/windDisplay.ts` (era duplicata localmente in `WindRibbon.tsx`, eliminata la duplicazione). **Opacità**: nuova `headwindOpacity()` in `windDisplay.ts`, proporzionale all'intensità del vento efficace rispetto al massimo sul percorso (minimo 0.22 così restano "soffuse" ma mai invisibili anche a vento debole) — combinazione colore+opacità scelta esplicitamente dall'utente. **Densità adattiva** (non passo fisso): ricalcolo a ogni `moveend`/`zoomend` della mappa (debounce 80ms) usando la formula standard Web Mercator metri/pixel alla latitudine e zoom correnti, target ~90px fra due frecce, clampato fra 120m e 6km di spaziatura per non degenerare a zoom estremi; solo i campioni dentro i bounds visibili vengono renderizzati (`bounds.contains`), con un tetto di sicurezza di 260 frecce. Le frecce sono gestite con un `L.LayerGroup` imperativo (non `Marker` React per ognuna) per tenere leggero il re-render a ogni pan. **Non ancora fatto** (esplicitamente fuori scope per questo primo giro, prossimo step naturale se richiesto): animazione delle frecce (vero effetto "flow"stile earth.nullschool), non implementata — l'utente ha chiesto esplicitamente di partire dalle frecce statiche come base. 102/102 test verdi (nessun nuovo test: componente puramente visivo/Leaflet, stesso pattern non testato di `WindMapControl`/`RouteMap`), typecheck pulito, build OK. File: `routesplitter-web.zip`.
- **2026-08-19 (2)** — Due richieste dell'utente dopo aver visto le frecce sulla mappa: "aggiungi il vento all'ottimizzatore" e "la visualizzazione testa/coda in altimetria è sballata".
  **Bug fix fascia vento in altimetria**: la causa era che `WindRibbon` viveva come componente separato SOPRA `ElevationChart`, a piena larghezza del proprio contenitore — mentre il grafico sottostante disegna in un `<svg viewBox>` con margini (55px sinistra per l'asse Y, 30px destra) e supporta lo zoom via brush (`zoomDomain`). La ribbon non teneva conto né dei margini (quindi era disallineata anche a riposo, di circa 6-9% della larghezza) né dello zoom (mostrava sempre l'intero percorso anche col grafico zoomato su un tratto: totalmente scollegata). Fix: la fascia vento non è più un componente a parte, è disegnata **dentro l'SVG di `ElevationChart`**, nello stesso `<g>` e con la stessa `xScale(d0,d1)` degli assi/profilo — quindi il colore sotto un punto del profilo corrisponde SEMPRE esattamente a quel punto, zoom incluso, per costruzione (non serve più tenerle sincronizzate a mano). Posizionata come striscia sottile (7px) nel margine superiore del grafico. Il colore/opacità restano quelli di `headwindColor`/`headwindOpacity` in `windDisplay.ts` (ora anche `headwindOpacity` estratta lì, riusata identica dalle frecce mappa), normalizzati sul massimo assoluto dell'INTERO percorso (non della sola porzione zoomata) così lo zoom non cambia cosa significa "rosso pieno". `WindRibbon.tsx` rimosso (sostituito, non più referenziato); resta solo una piccola legenda testuale sotto il grafico. `ElevationChart` ha ora un prop opzionale `windZones`.
  **Vento nell'ottimizzatore di pacing**: `OptimizableSegment` e `PowerSegment` (physics-core/types.ts) hanno un nuovo campo opzionale `windKmh` — se assente, comportamento storico invariato (usa `params.windKmh` scalare globale). `pacingOptimizer.ts` e `normalizedPower.ts` costruiscono ora dei "params effettivi per segmento" (`paramsForSegment`, stesso pattern in entrambi i file) che sovrascrivono `windKmh` con quello del segmento quando presente, in OGNI punto dove prima si chiamava `speedFromPower(..., params)` a occhio globale (loop marginale, proiezione sulla media pesata, aggiustamento NP, calcolo finale). `lib/pacingActions.ts`: `breakpointsToSegments` e `buildFineGrid` accettano ora un `windZones?` opzionale e calcolano il vento efficace al **punto medio** di ciascun segmento (`windAtDistKm` + `routeBearingAtDistKm` + `effectiveHeadwindKmh`) — stessa approssimazione già usata da `computeSections` per le sezioni utente, nessun doppio standard fra "sezioni" e "ottimizzatore" per lo stesso tratto di percorso. `PowerPlanModal.tsx` (il grafico potenza prevista) applica lo stesso vento per-segmento nella sua serie temporale, altrimenti il grafico mostrerebbe un tempo/velocità diverso da quello con cui l'ottimizzatore ha effettivamente calcolato i watt. `PacingOptimizerPanel` mostra un badge "🌬️ con vento" quando ci sono ≥2 zone vento definite.
  **Comportamento fisico verificato con un test dedicato** (non ovvio, verificato empiricamente prima di scrivere l'asserzione — la prima intuizione era sbagliata): a parità di pendenza e di potenza media target, l'ottimizzatore assegna **più** potenza al tratto in forte vento in testa e **meno** a quello in coda, non il contrario. Motivo: in testa la velocità di equilibrio è molto più bassa (regime aerodinamico meno dominante, quasi-lineare, come in salita), quindi un watt marginale lì compra proporzionalmente più tempo risparmiato; in coda si è già veloci, nel regime aerodinamico quasi-cubico a rendimenti fortemente decrescenti, dove un watt marginale compra pochissimo. Stesso principio già validato per salita-vs-piano, qui è lo stesso effetto guidato dal vento invece che dalla pendenza — ed è la strategia di pacing corretta (spingere contro vento, tirare il fiato in coda). Aggiunti test in `pacingOptimizer.test.ts` (comportamento wind-aware + fallback storico invariato senza windKmh) e in `pacingActions.test.ts` (propagazione corretta del vento per-segmento, incluso un caso a due zone con cambio di direzione a metà percorso). 107/107 test verdi, typecheck pulito, build OK. File: `routesplitter-web.zip`.
- **2026-08-19 (3)** — Su richiesta dell'utente ("si", confermando la proposta di procedere con l'animazione), le frecce statiche sulla mappa sono state **sostituite** da un'animazione vento in stile earth.nullschool/Windfinder. Nuovo componente `WindFlowLayer.tsx` (rimpiazza `WindArrowsLayer.tsx`, eliminato): un unico `<canvas>` (non Marker/divIcon per particella — a 60fps con ~130 particelle il DOM andrebbe in crisi) aggiunto come figlio diretto del container Leaflet (non di un pane interno: le posizioni si ricalcolano da lat/lon reali ad ogni frame via `map.latLngToContainerPoint`, quindi pan/zoom restano corretti senza gestire a mano le trasformazioni CSS dei pane), ridisegnato ogni `requestAnimationFrame`. ~130 particelle scorrono nella direzione vera verso cui soffia il vento locale (formula di destinazione sferica standard, stesso bearing "blowing to" già usato da bussola/frecce), a una velocità proporzionale all'intensità REALE del vento (non alla sola componente in testa/coda) — normalizzata sul massimo assoluto di velocità vento sull'intero percorso, non solo la vista corrente, per stabilità durante pan/zoom. Velocità apparente sullo schermo (16-62 px/s) scalata coi metri/pixel correnti così resta comparabile a qualsiasi zoom, stesso principio già usato per la densità delle vecchie frecce. Ogni particella lascia una scia corta (6 punti) con opacità decrescente verso la coda ("effetto cometa"); niente trucco di compositing con canvas semi-trasparente (avrebbe richiesto uno sfondo di colore fisso, qui sotto c'è la mappa vera) — si ridisegna per intero ogni frame. Colore/opacità della scia restano rosso/verde per intensità in testa/coda, stessa scala condivisa (`headwindColor`/`headwindOpacity`) di fascia in altimetria e badge tabella — nessuna nuova palette introdotta, coerenza con D20-D22 (niente redesign non richiesti). Punti di semina ricalcolati su `moveend`/`zoomend` (debounce 80ms) con spaziatura adattiva come le vecchie frecce; dispersione laterale casuale (~140m) attorno al punto per dare l'idea di un "corridoio" di vento invece di una singola linea. Ogni particella si rigenera (nuovo punto di semina, nuova durata 3.5-7s casuale) a fine vita o se esce dalla vista con un margine del 15%. Animazione in pausa quando il tab non è visibile (`visibilitychange`) per risparmiare batteria; rispetta `prefers-reduced-motion` (nessuna animazione, componente non monta nulla). CSS `.wind-flow-canvas` con `z-index: 350` — sotto il layer overlay/route (400) e i marker (600), sopra le tile (200): le particelle scorrono "dietro" al tracciato e ai breakpoint, mai sopra, per non disturbare la leggibilità del percorso. Nessun nuovo test (stesso pattern non testato di `WindMapControl`/vecchio `WindArrowsLayer`: componente puramente visivo/canvas/rAF, non testabile in modo significativo con vitest+jsdom). 107/107 test verdi (nessuna regressione, solo sostituzione di un componente di visualizzazione), typecheck pulito, build OK. File: `routesplitter-web.zip`.
- **2026-08-19 (4)** — Bug fix: dopo aver visto l'animazione vento in dev, l'utente segnalava "non si vedono queste nuove frecce". Diagnosi guidata a distanza (niente accesso al browser dell'utente): confermato prima wind speed >0 e console pulita, poi test diagnostico decisivo (un quadrato di debug disegnato ogni frame indipendentemente dalle particelle) — visibile SOLO per un istante durante l'animazione di zoom-indietro, mai a mappa statica o zoom-in. Causa reale: il `z-index: 350` scelto per `.wind-flow-canvas` non teneva conto che il pannello principale di Leaflet (`mapPane`, contiene tile+percorso+marker) porta la classe generica `.leaflet-pane` con `z-index: 400` applicata all'INTERO pannello esterno, non solo ai sotto-pannelli interni (avevo sbagliato a considerare solo z-index interni come tilePane/overlayPane/markerPane, dimenticando che il wrapper esterno stesso è già a 400) — quindi il canvas restava sistematicamente DIETRO tutto il contenuto della mappa, rivelato solo per un istante quando Leaflet rimpicciolisce temporaneamente il pannello durante l'animazione di zoom. Fix: `z-index: 690` in `index.css` (sopra tutti i pannelli standard fino a popupPane 700 escluso, sotto i controlli Leaflet a 1000). Nessuna modifica alla logica del componente (particelle/spawnPoints erano già corretti: confermato 130/130 particelle vive, 8-9 spawnPoints). Rimossa la strumentazione di debug temporanea (log console + quadrato magenta) usata per isolare il problema. 107/107 test verdi, typecheck pulito, build OK. File: `routesplitter-web.zip`.
- **2026-08-19 (5)** — Feedback dell'utente dopo aver visto l'animazione funzionante: "sono orribili, sembrano schizzate a caso... voglio qualcosa come Windfinder, linee che si muovono". Riprogettazione completa del rendering (stessa infrastruttura canvas/z-index, cambiata solo la logica di disegno): **rimossa** la vecchia versione a particelle (spawn con posizione casuale sparsa attorno al punto di semina — 140m di jitter random ad ogni respawn, causa dell'effetto "a spruzzo" segnalato — e ciclo di vita con età/respawn/trail di storia). **Sostituita** con un modello stateless a linee tratteggiate stile Windfinder: ogni punto campionato lungo il percorso (densità adattiva a zoom/pan, invariata) porta 3 "corsie" a offset FISSO (-15/0/+15px, perpendicolari alla direzione del percorso — nessuna casualità), ciascuna un segmento con tratteggio (`ctx.setLineDash`) che scorre continuamente via `lineDashOffset` animato nel tempo (nessuno stato di "particella" da tracciare — ogni frame ridisegna da zero (tempo trascorso, vento locale), deterministico). Piccola freccia piena in fondo a ogni segmento per rinforzare la direzione. **Iterazione sul colore**: primo tentativo cambiato a scala per intensità (calmo→ciano→forte, dedicata `windIntensityColor`) su richiesta implicita, poi l'utente ha chiarito di preferire il colore ORIGINALE per direzione (rosso/verde testa/coda, `headwindColor` — stesso motivo iniziale: comunica l'effetto sulla marcia, coerente con fascia altimetria/tabella) — `windIntensityColor`/`windIntensityOpacity` aggiunte e poi rimosse nella stessa sessione (mai arrivate a un utente finale, nessun costo di migrazione). **Resta invece** dall'iterazione sull'intensità: la VELOCITÀ di scorrimento del tratteggio è legata all'intensità REALE del vento (velocità grezza, non componente testa/coda) — vento calmo scorre piano, vento forte scorre veloce, richiesta esplicita confermata ("tipo Windfinder"). Nessun nuovo test (stesso pattern non testato di prima: canvas/rAF puro). 107/107 test verdi, typecheck pulito, build OK. File: `routesplitter-web.zip`.
- **2026-08-19 (6)** — Feedback dell'utente: "queste sono frecce fisse, mica si muovono". Non era un bug funzionale (il `lineDashOffset` cambiava correttamente ogni frame) ma un problema di percettibilità: su un segmento fisso di 44px con trattini larghi (9px trattino/7px vuoto), lo scorrimento del tratteggio è troppo sottile per l'occhio umano — sembra statico anche se tecnicamente non lo è. Fix: pista più lunga (64px), puntini piccoli (3px) con vuoti larghi (16px) invece di trattini larghi ravvicinati — un singolo puntino che percorre un tratto lungo si vede chiaramente, trattini larghi che si "confondono" fra loro frame dopo frame no. Aggiunta anche una guida statica sotto ai puntini animati (stessa linea, senza tratteggio, opacità al 30%) così la direzione resta leggibile a colpo d'occhio anche a vento debole/animazione lenta, con i puntini sopra a fare da segnale di movimento. Range di velocità alzato (24-130 px/s, prima 14-90) perché a bassa intensità il movimento deve restare comunque percettibile. 107/107 test verdi, typecheck pulito, build OK. File: `routesplitter-web.zip`.
- **2026-08-19 (7)** — Tre richieste dell'utente: "le frecce sono ferme, è solo il tratteggio interno che si muove — voglio righe che si muovono per tutta la mappa come Windfinder", "l'intensità è già massima a 5 km/h", "convertitore km/h↔kts nel pannello vento".
  **Movimento reale nello spazio**: la versione precedente (tratteggio via `lineDashOffset` dentro un segmento fisso) dava l'idea giusta ma il movimento restava confinato in un'area troppo piccola per essere percepito come "flusso" — l'utente lo vedeva giustamente come fermo. Riprogettato daccapo: ogni corsia (posizione fissa, nessuna casualità — la primissima versione a particelle sparse dava l'effetto "a spruzzo" già corretto in sessione precedente) ora ospita 3 streak sfalsati che percorrono REALMENTE un loop di ~170px sullo schermo, con dissolvenza seno in ingresso/uscita per nascondere il salto di fine giro — vero movimento visibile "per tutta la mappa" (nei limiti della corsia), non un effetto ottico confinato. Nessuna geometria reale (lat/lon) coinvolta nel movimento stesso: tutto calcolato in spazio schermo (pixel), l'origine di ogni punto-vento viene ricalcolata da lat/lon reali a ogni frame via `map.latLngToContainerPoint` (come da sempre), quindi pan/zoom restano corretti.
  **Scala assoluta invece che relativa**: colore/opacità/velocità normalizzavano sul massimo trovato SUL PERCORSO — un percorso con vento max 5-8 km/h mostrava già rosso pieno a 5 km/h (corretto rispetto al proprio massimo, sbagliato in assoluto: 5 km/h non è vento forte). Sostituito con riferimenti fissi in km/h (`REFERENCE_MAX_HEADWIND_KMH = 30`, `REFERENCE_MAX_SPEED_KMH = 30`) — come fa Windfinder con la sua scala fissa, non auto-tarata sui dati correnti. Rimossa la scansione dell'intero percorso per calcolare i massimi (non più necessaria, componente più semplice).
  **Colore**: confermato di nuovo per DIREZIONE (testa/coda rosso/verde, `headwindColor`) — non cambiato in questo giro, l'utente aveva già chiarito la preferenza nel giro precedente.
  **Convertitore km/h↔kts**: nuovo componente `WindSpeedConverter` dentro `WindZonesPanel.tsx`, due campi numerici bidirezionali (digitare in uno aggiorna l'altro, 1 nodo = 1.852 km/h), in cima al pannello vento, sempre visibile. Stato locale indipendente dalle zone vento (puro calcolatore, non scrive/legge dati del piano).
  Nessun nuovo test (componente canvas/rAF, stesso pattern non testabile significativamente di prima; il convertitore è UI pura senza logica di dominio da testare oltre a un semplice fattore di conversione). 107/107 test verdi, typecheck pulito, build OK. File: `routesplitter-web.zip`.
- **2026-08-19 (8)** — Rifinitura dopo il feedback sullo streamline: "linee più fine, stile Windfinder", "a 5km/h sono 0 visibili — voglio più intenso: più veloce = colore per testa/coda, ma trasparenza sempre uguale e ben visibile". Due modifiche in `WindFlowLayer.tsx`: (1) `LINE_WIDTH` ridotto 2.4→1.3 (linee sottili, non più vistose); (2) l'opacità non dipende più dall'intensità del vento (era `headwindOpacity(headwindKmh, REFERENCE_MAX_HEADWIND_KMH)`, quindi a vento debole quasi invisibile) — ora è una costante fissa `BASE_OPACITY = 0.92`, sempre uguale indipendentemente da quanto soffia. La codifica dell'intensità resta SOLO nella velocità di scorrimento (già così dal giro precedente); la direzione resta SOLO nel colore (`headwindColor`, invariato). Tre canali visivi ora ben separati: colore=direzione, velocità=intensità, trasparenza=costante/sempre leggibile — esattamente la richiesta. Rimosso l'uso di `headwindOpacity` in questo file (resta invariato altrove: fascia altimetria e badge tabella). 107/107 test verdi, typecheck pulito, build OK. File: `routesplitter-web.zip`.
- **2026-08-19 (9)** — Feedback dell'utente con screenshot: "il grigio non si vede sopra OpenStreetMap, e a 5km/h è tutto grigio" + "riduci la grandezza dei numeri delle sezioni sulla mappa, almeno la metà".
  **Contrasto vento su mappa**: due cause distinte, corrette entrambe in `WindFlowLayer.tsx`. (1) `REFERENCE_MAX_HEADWIND_KMH` era 30 — per venti reali tipici (5-15 km/h, il caso più comune) anche una componente in testa piena restava quasi tutta grigia perché lontana dal riferimento di saturazione: abbassato a 12 km/h, così il colore comunica la direzione con decisione anche a intensità realistiche, non solo a vento fortissimo. (2) Il grigio (vento in traverso, componente vicina a zero) è oggettivamente illeggibile sopra tile OpenStreetMap chiare/verdi senza un bordo di contrasto — aggiunto un ALONE scuro dietro ogni linea (guida + streak, `HALO_COLOR = 'rgba(15,23,42,0.65)'`, spessore +1.6px rispetto alla linea colorata sopra), tecnica cartografica standard (bordi delle piste sugli atlanti sciistici) che garantisce leggibilità su qualunque sfondo di mappa, non solo grigio.
  **Marker numerati delle sezioni**: `.map-num-icon` in `index.css` (i cerchi numerati 1,2,3... che segnano i breakpoint sulla planimetria) ridotti da 22px a 12px di diametro (font 11px→7px, bordo 2px→1px) — più della metà come richiesto, minimo che resti leggibile. Icon size/anchor di Leaflet aggiornati in `RouteMap.tsx` di conseguenza.
  Nessun nuovo test (modifiche visive canvas/CSS, stesso pattern non testabile di prima). 107/107 test verdi, typecheck pulito, build OK. File: `routesplitter-web.zip`.
- **2026-08-19 (10)** — Segnalazione grave con screenshot: "sono le più brutte viste fino ad ora, sembrano dei grossi treni, non si vede manco il percorso". Causa reale (non solo estetica): la versione precedente disegnava, per ogni punto-vento, 3 corsie parallele × (1 linea guida di sfondo lunga 170px + 3 streak) = 12 tratti, con punti ogni 120px. Sui tratti con tornanti ravvicinati (segmenti di percorso geograficamente vicini ma lontani in km lungo il tracciato, tipici delle salite — visibili nello screenshot dell'utente) le linee guida da 170px di punti adiacenti si sovrapponevano abbondantemente, producendo bande diagonali continue che coprivano il percorso.
  Riprogettazione drastica (semplificazione, non un altro strato di complessità): un solo streak per punto (non 3 corsie), nessuna linea guida di sfondo (era la causa principale), oscillazione locale breve (~26px, non un tragitto lungo attraverso la mappa) che non esce mai dal proprio "territorio" — niente più sovrapposizioni fra punti vicini qualunque sia la geometria del percorso, tornanti inclusi. Densità dimezzata circa (spaziatura target 120→200px, max punti 70→45). Linea più sottile (1px + alone 0.9px, prima 1.3px + alone 1.6px). Colore per direzione, velocità per intensità, trasparenza costante: tutti confermati invariati dal giro precedente, solo l'impianto geometrico è cambiato. Molto più vicino a Windfinder (segni piccoli, sottili, radi) di ogni tentativo precedente.
  Nessun nuovo test (canvas/rAF, stesso pattern non testabile di prima). 107/107 test verdi, typecheck pulito, build OK. File: `routesplitter-web.zip`.
